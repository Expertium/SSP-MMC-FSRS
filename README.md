...
5) When the user can't keep up and a review backlog forms (e.g. at high desired retention, where the daily study-time budget binds and starves new-card learning), the simulator admits due cards in card-introduction order, doing reviews before new cards. It does not model Anki's review-order options ("show cards with longer intervals first", "highest probability of recall first", random, etc.). This only matters in backlog scenarios — when every due card is reviewed each day the order is irrelevant. (Because every policy, including the fixed-retention baselines, is simulated under the identical rule, the *relative* comparisons are unaffected; only absolute numbers in the backlog regime depend on the order.)

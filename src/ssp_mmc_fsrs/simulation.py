    ...
    try:
        ...
        try:
            ...
            try:
                ...
                raise TypeError(
                    f"Policy {policy.__name__ if hasattr(policy, '__name__') else str(policy)} "
                    "doesn't support any of the expected signatures: "
                    "5 variables (stability, difficulty, prev_interval, grade, ease), "
                    "4 variables (stability, difficulty, prev_interval, grade), or "
                    "2 variables (stability, difficulty)"
                )
    ...
    for today in trange(learn_span, position=1, leave=False, desc=progress_desc):
        ...
        if true_review.any():
            ...
            if isinstance(policy_result, tuple):
                ...
                new_ivl_review[review_mask] = new_intervals.float()
                new_ease_review[review_mask] = new_eases.float()
            else:
                new_ivl_review[review_mask] = policy_result.float()
        ...
        if true_learn.any():
            ...
            initial_ease = torch.full_like(ease[learn_mask], 2.5)
            ...
            if isinstance(policy_result, tuple):
                new_intervals, new_eases = policy_result
                new_ivl_learn[learn_mask] = new_intervals.float()
                new_ease_learn[learn_mask] = new_eases.float()
            else:
                new_ivl_learn[learn_mask] = policy_result.float()
